...
#' @param e1: column containing efficiency values, usually [5] when open_data.R has been followed and status deleted
...
#' @param row[5]: refers to the fifth column containing the efficiency values
#' 
#' @example 
...
#' @param d1: column containing delta ct previously calculated, normally [6]
...
#' @param row[6]: refers to the sixth column containing the delta ct values
#' 
#' @example 
...
taq_data$ratio <- apply(taq_data, 1, function(row)  ratio_pfaffl(taq_data, row[1], row[6]))
